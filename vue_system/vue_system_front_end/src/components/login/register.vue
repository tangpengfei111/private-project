<template>
    <el-dialog width='420px' :visible="flag" top='8vh' align='center' :before-close="handleClose">
        <el-form :model=" formData" status-icon :rules="rules" ref="formData" label-width="100px">
            <div class="title">普通管理员注册</div>
    ...
    export default {
        ...
        data() {
            var conformName = (rule, value, callback) => {
                ...
                }else if (this.formData.username.length > 5 && this.formData.username.length < 17) {
                    callback();
                }else {
            ...
            var validatePass = (rule, value, callback) => {
                if (value === '') {
                    ...
                    if (this.formData.password.length < 5 || this.formData.password.length > 17) {
            ...
            var checkMail = (rule, value, callback) => {
                ...
                }else if (/^(\w)+([-.]\w+)*@[a-zA-Z0-9]+(\.[a-zA-Z0-9])*\.[a-zA-Z]{2,5}$/.test(this.formData.mail)) {
                    callback();
                }else {
            ...
            return {
                ...
                rules: {
                    username: [
                        { required: true, validator: conformName, trigger:'blur' }
                    ],
                    password: [
